/**
 * Runs scripts/sql/verify-purchase-returns.sql against the Postgres `db` service of the
 * Docker Compose stack. The SQL asserts that confirming a purchase return takes stock
 * back out, that over-returns are rejected, that negative stock is allowed and reported,
 * and that cost is untouched; it rolls back, so nothing is left behind.
 *
 * Prereq: stack up (`docker compose up -d`) with migrations applied
 * (`npm run db:push:docker-compose`).
 *
 * Pass --with-migration to prepend 20260903120000_purchase_returns.sql inside the same
 ...
 *
 * Usage:
 *   node scripts/verify-purchase-returns.mjs
 *   node scripts/verify-purchase-returns.mjs --with-migration
 *   node scripts/verify-purchase-returns.mjs --env-file .env.docker.shop
 */
...
const checks = readFileSync(
  join(root, 'scripts', 'sql', 'verify-purchase-returns.sql'),
  'utf8'
)
  ...
  ? [
      ...
      readFileSync(
        join(
          ...
          'supabase',
          'migrations',
          '20260903120000_purchase_returns.sql'
        ),
        'utf8'
      ),
...
if (result.status !== 0) {
  console.error('\nverify-purchase-returns FAILED.')
  process.exit(result.status ?? 1)
}

console.log('\nverify-purchase-returns passed.')
