...
import { getAllReturns, getSourceOrderNumbers } from '@/services/returnService'
import { listWarehouses } from '@/services/warehouseService'
import type { ReturnStatusFlow } from '@/types'
...
import { ReturnCsvImportDialog } from '@/components/returns/ReturnCsvImportDialog'
import { useFeatureEnabled } from '@/context/FeatureControlContext'
import { downloadCsv } from '@/utils/csvDownload'
import { flattenReturnsForCsvExport } from '@/utils/returnCsvImport'

type StatusTab = 'all' | ReturnStatusFlow
...
export function Returns() {
  ...
  const hubList = useFeatureEnabled('orders.returnsHubList')
  const canImportCsv = useFeatureEnabled('orders.returnImportCsv')
  const canExportCsv = useFeatureEnabled('orders.returnExportCsv')
  ...
  useEffect(() => {
    document.title = `${t('returns.title')} | StockPilot`
    return () => {
  ...
  const { data: returnsRaw = [], isLoading: listLoading } = useQuery({
    queryKey: ['returns', 'list', debouncedSearch],
    queryFn: () =>
      getAllReturns({ search: debouncedSearch.trim() || undefined }),
  })
  ...
  const sourceOrderIds = useMemo(
    () => returnsRaw.map((r) => r.source_order_id),
    [returnsRaw]
  )

  const { data: sourceNumbers } = useQuery({
    queryKey: ['returnSourceOrderNumbers', sourceOrderIds],
    queryFn: () => getSourceOrderNumbers(sourceOrderIds),
  ...
  const exportReturnsCsv = useCallback(() => {
    const rows = flattenReturnsForCsvExport(filteredList, {
  ...
  const onCsvImportComplete = useCallback(() => {
    ...
    void queryClient.invalidateQueries({ queryKey: ['returnableLines'] })
  }, [queryClient])
    ...
    return (
      ...
      >
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          {t('control.disabled.returnsList')}
        </p>
      </div>
    )
  }
      ...
      <div className="flex gap-1 overflow-x-auto border-b px-2 py-2">
        {(
          [
            ...
            ['draft', counts.draft, 'returns.draft'],
            ['confirmed', counts.confirmed, 'returns.confirmed'],
            ['cancelled', counts.cancelled, 'returns.cancelled'],
          ] as const
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {listLoading ? (
          ...
          <p className="p-6 text-center text-sm text-muted-foreground">
            {t('returns.emptyReturns')}
          </p>
        ) : (
          <ul className="mx-auto flex max-w-2xl flex-col gap-2">
            {filteredList.map((r) => {
              const orderNumber = sourceNumbers?.get(r.source_order_id)
              return (
                <li key={r.id}>
                  <Link
                  ...
                  >
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
                      {orderNumber != null && (
                        <span>
                          {t('returns.againstOrder')} #{orderNumber}
                        </span>
                      )}
                      <span>
                        {r.items.length === 1
                    ...
                    <div className="mt-2 space-y-0.5 border-t pt-2 text-xs tabular-nums">
                      ...
                      {r.settlement && (
                        <div className="flex justify-between text-muted-foreground">
                          <span>{t('returns.settlementLabel')}</span>
