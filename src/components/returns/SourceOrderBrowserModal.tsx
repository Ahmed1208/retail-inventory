...
type Props = {
  ...
  /** Already filtered to orders that can be returned against. */
  orders: OrderWithItemsAndPayments[]
  people: Person[]
  isRTL: boolean
  lang: 'en' | 'ar'
  formatCurrency: (n: number) => string
  onPick: (order: OrderWithItemsAndPayments) => void
}

export function SourceOrderBrowserModal({
  open,
  onOpenChange,
  orders,
  people,
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter((o) => {
      ...
      const name = o.person_id ? nameById.get(o.person_id) : undefined
      ...
      >
        ...
        <div className="relative shrink-0 border-b bg-muted/30 px-4 py-3">
          ...
          <Input
            ...
            placeholder={t('returns.searchSourceOrders')}
            className="ps-9"
          />
        </div>
              ...
              {filtered.map((o, i) => (
                ...
                >
                  ...
                  <div className="mt-0.5 text-xs text-muted-foreground">
                    {o.person_id
                      ? (nameById.get(o.person_id) ?? t('orders.walkIn'))
                      : t('orders.walkIn')}
                  </div>
                  <div className="mt-0.5 text-xs text-muted-foreground">
                    {dateFmt.format(new Date(o.created_at))} ·{' '}
                    {t('returns.linesCount', { count: o.items.length })}
                  </div>
                </button>
              ))}
            </div>
